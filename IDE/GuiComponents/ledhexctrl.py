...
class LEDHexCtrl(wx.Control):
    """
    The LEDHexCtrl can be used to display a series of hex digits, (plus spaces,
    ...
    class const:
        ...
        DIGITS = {
            ...
            'A': LINE1 | LINE2 | LINE3 | LINE5 | LINE6 | LINE7,
            'B': LINE3 | LINE4 | LINE5 | LINE6 | LINE7,
            'C': LINE1 | LINE4 | LINE5 | LINE6,
            'D': LINE2 | LINE3 | LINE4 | LINE5 | LINE7,
            'E': LINE1 | LINE4 | LINE5 | LINE6 | LINE7,
            'F': LINE1 | LINE5 | LINE6 | LINE7,
            '-': LINE7,
            ':': COLON,
        }
        DIGITALL = 0xFFFF
    ...
    def _init_default(self):
        super(LEDHexCtrl, self).__init__()
        self._init()
    ...
    def SetValue(self, value, redraw=True):
        ...
        if value != self.m_value:
            for ch in value:
                assert ch in '0123456789ABCDEF-.: ', "LEDHexCtrl can only display hex numeric string values."
    ...
    def OnPaint(self, evt):
        ...
        for i, ch in enumerate(self.m_value):
            ...
            if self.m_drawFaded and ch != '.':
                self._drawHexDigit(dc, c.DIGITALL, i);

            if ch == '.':
                # draw the decimal point in the previous segment
                self._drawHexDigit(dc, c.DECIMALSIGN, i - 1)
                offset += 1
            elif ch == ' ':
                # skip spaces
                continue
